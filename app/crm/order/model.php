...
class orderModel extends model
{
    ...
    public function getList($mode = 'all', $param = '', $owner = '', $needQueryCondition = true, $orderBy = 'id_desc', $pager = null)
    {
        ...
        if(strpos(',contactedby,past,today,tomorrow,thisweek,thismonth,', ",{$mode},") !== false or ($mode == 'bysearch' && strpos($orderQuery, '`nextDate`') !== false))
        {
            $orders = $this->dao->select('o.*, c.name AS customerName, c.level AS level')->from(TABLE_ORDER)->alias('o')
                ->leftJoin(TABLE_CUSTOMER)->alias('c')->on('o.customer=c.id')
                ->leftJoin(TABLE_DATING)->alias('d')->on('o.id=d.objectID')
                ->where('o.deleted')->eq(0)
                ->andWhere('d.status')->eq('wait')
                ->andWhere('d.objectType')->eq('order')
                ->beginIF($mode == 'contactedby')->andWhere('d.account')->eq($this->app->user->account)->fi()
                ->beginIF($mode == 'past')->andWhere('d.date')->ne('0000-00-00')->andWhere('d.date')->lt(helper::today())->andWhere('o.status')->ne('closed')->fi()
                ->beginIF($mode == 'today')->andWhere('d.date')->eq(helper::today())->fi()
                ->beginIF($mode == 'tomorrow')->andWhere('d.date')->eq(formattime(date::tomorrow(), DT_DATE1))->fi()
                ->beginIF($mode == 'thisweek')->andWhere('d.date')->between($thisWeek['begin'], $thisWeek['end'])->fi()
    ...
    public function buildOperateMenu($order)
    {
        ...
        $menu .= commonModel::printLink('action', 'createRecord', "objectType=order&objectID={$order->id}&customer={$order->customer}", $this->lang->order->record, "data-toggle='modal' data-width='800'", false);
